...
class Orbital:
    def __init__(self, orbiting_mass, central_mass, initial_position, initial_velocity, time, step):
        ...
        # defines starting postion of orbiting mass relative to central mass
    ...
    def k_pos_vel(self, pos_x, pos_y, vel_x, vel_y):
        ...
        k2vel_x = self.xvelocity(
            pos_x + (self.step * k1pos_x) / 2, pos_y + (self.step * k1pos_y) / 2)
        k2vel_y = self.yvelocity(
            pos_x + (self.step * k1pos_x) / 2, pos_y + (self.step * k1pos_y) / 2)
        ...
        k3vel_x = self.xvelocity(
            pos_x + (self.step * k2pos_x) / 2, pos_y + (self.step * k2pos_y) / 2)
        k3vel_y = self.yvelocity(
            pos_x + (self.step * k2pos_x) / 2, pos_y + (self.step * k2pos_y) / 2)
        ...
        k4vel_x = self.xvelocity(
            pos_x + self.step * k3pos_x, pos_y + self.step * k3pos_y)
        k4vel_y = self.yvelocity(
            pos_x + self.step * k3pos_x, pos_y + self.step * k3pos_y)
    ...
    def tstepping_position(self, pos_x, pos_y, kpos_xlist, kpos_ylist):
        ...
        pos_x = pos_x + \
            (self.step / 6) * \
            (kpos_xlist[0] + 2 * kpos_xlist[1] +
             2 * kpos_xlist[2] + kpos_xlist[3])
        pos_y = pos_y + \
            (self.step / 6) * \
            (kpos_ylist[0] + 2 * kpos_ylist[1] +
             2 * kpos_ylist[2] + kpos_ylist[3])
        return pos_x, pos_y
    ...
    def tstepping_velocity(self, vel_x, vel_y, kvel_xlist, kvel_ylist):
        ...
        vel_x = vel_x + \
            (self.step / 6) * \
            (kvel_xlist[0] + 2 * kvel_xlist[1] +
             2 * kvel_xlist[2] + kvel_xlist[3])
        vel_y = vel_y + \
            (self.step / 6) * \
            (kvel_ylist[0] + 2 * kvel_ylist[1] +
             2 * kvel_ylist[2] + kvel_ylist[3])
        return vel_x, vel_y
    ...
    def simulate_orbit(self):
        ...
        for i in range(0, len(time_list)):
            kpos_xlist, kpos_ylist, kvel_xlist, kvel_ylist = self.k_pos_vel(
                pos_x, pos_y, vel_x, vel_y)
            pos_x, pos_y = self.tstepping_position(
                pos_x, pos_y, kpos_xlist, kpos_ylist)
            vel_x, vel_y = self.tstepping_velocity(
                vel_x, vel_y, kvel_xlist, kvel_ylist)
    ...
    def animation(rx, ry, rdotx, rdoty):
        ...
        # values from the numerical method are too large for the canvas
        scale_x = (window_width - 100) / (2 * rx.max() - rx.min())
        # and so are scaled so that the animation can be viewed properly
        scale_y = (window_height - 100) / (2 * ry.max() - ry.min())
        rx *= scale_x  # the following lines apply the scale to the simulated results
        ry *= -scale_y
        rx += center_x
        ry += center_y
        # initialises the canvas with desired specifications
        window = tk.Canvas(root, width=window_width,
                           height=window_height, bg="#02198c")
        window.pack()
        radius_e = 10  # radius for earth tkinter object
        radius_s = 5  # radius for satellite tkinter object
        x0e, y0e, x1e, y1e = center_x - radius_e, center_y - radius_e, center_x + \
            radius_e, center_y + radius_e  # next two lines position the objects
        # in the canvas so they are visible
        x0s, y0s, x1s, y1s = center_x, center_y, center_x, center_y
        # initalises the earth and moon object with desired
        earth = window.create_oval(
            x0e, y0e, x1e, y1e, fill="white", outline="white")
        # positions and colour scheme
        satellite = window.create_oval(
            x0s, y0s, x1s, y1s, fill="white", outline="white")
        # creates radial line from central to orbital mass
        radial_line = window.create_line(
            center_x, center_y, center_x, center_y, fill="white")
        # creates a rectangular box used for showing stats
        stats = window.create_rectangle(
            10, window_height - 5, 200, window_height - 45, fill="white", outline="")
        # creates text that names the central mass on canvas
        earth_text = window.create_text(
            center_x + 10, center_y + 10, text="EARTH", fill="white", anchor="nw")
        # creates text that follows the orbiting mass
        sat_text = window.create_text(
            rx[0] + 10, ry[0] + 10, text="SATELLITE", fill="white", anchor="s")
        polygon_coords = []  # creates and array that will hold orbiting mass position
        # for loop appends the orbiting mass position to the polygon_coords array
        for i in range(0, len(rdotx)):
            polygon_coords.append(rx[i])
            polygon_coords.append(ry[i])
        # creates a polygon in the canvas that shows the trajectory of the orbiting mass
        sat_path2 = window.create_polygon(
            polygon_coords, outline="white", dash="-.-", fill="")
        counter = 0
        while counter <= len(rdotx) + 1:
            if counter == len(rdotx):
                counter = 0
            window.coords(satellite, rx[counter] - radius_s, ry[counter] - radius_s, rx[counter] +
                          radius_s, ry[counter] + radius_s)  # moves the orbiting mass accordingly
            # moves the sat_text to follow the orbiting mass text
            window.coords(sat_text, rx[counter], ry[counter] - 10)
            # moves radial line from central to orbital mass
            window.coords(radial_line, center_x, center_y,
                          rx[counter], ry[counter])
            distance = np.sqrt(((center_x - rx[counter]) ** 2) + ((center_y - ry[counter]) ** 2)) * 2 / (
                scale_x + scale_y)  # determines the distance between the orbiting and central mass
            if distance < 6371e3:
                # if distance between central and orbiting mass is smaller than the radius of the central mass, assume orbiting mass has crashed
                print("Unstable Orbit, Satellite has crashed!")
                window.delete(stats)
                break  # closes the tkinter window
            # next 7 lines of code display the stats of the orbiting mass in the stats box; distance from central mass and velocity
            velocity = np.sqrt(rdotx[counter] ** 2 + rdoty[counter] ** 2)
            velocity, distance = "%.4f" % velocity, "%.4f" % distance
            radial_dist = window.create_text(
                15, window_height - 25, text="Radial Distance : " + distance + " m", fill="#02198c", anchor="sw")
            tang_velocity = window.create_text(
                15, window_height - 10, text="Velocity : " + velocity + " m/s", fill="#02198c", anchor="sw")
...
def main():
    ...
    satellite_velocity = 2.07e3  # metres per second
    multiplier = 1  # amount of days the simulation should run
    satellite = Orbital(satellite_mass, earth_mass,
                        [0, orbital_radius],
